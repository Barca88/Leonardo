    ...
    <div id="users">
        ...
        ><template v-slot:top>
            <v-toolbar flat color="white">
                <v-toolbar-title>{{$t('users.titleAlu')}}</v-toolbar-title>
                <v-divider
                class="mx-4"
                inset
                vertical
...
export default {
    ...
    methods: {
      atualizarInfo: function(){
        this.dialog=false
        axios.get(`${process.env.VUE_APP_BACKEND}/users/users?type=Student`, { headers: { Authorization: `Bearer: ${this.$store.state.jwt}` } })
          .then(response => {
      ...
      verObjectItem(item,value){
        ...
        if (value == 'curriculo'){
          axios.get(`${process.env.VUE_APP_BACKEND}/users/curriculo/${this.users[index]._id}?nome=${this.$store.state.user._id}&?seed=${Date.now()}&action=gestao&type=Student  `, {
          ...
          .then(response => {
            ...
            axios.get(`${process.env.VUE_APP_BACKEND}/users/foto/${this.users[index]._id}?nome=${this.$store.state.user._id}&?seed=${Date.now()}&action=gestao&type=Student`, {
                responseType:'arraybuffer',
                headers: {
                    'Authorization': `Bearer: ${this.$store.state.jwt}`
                }
      ...
      deleteItem (item) {
        ...
        axios.get(`${process.env.VUE_APP_BACKEND}/users/apagar/` + this.users[index]._id + `?nome=${this.$store.state.user._id}&type=Student`,{ headers: { Authorization: `Bearer: ${this.$store.state.jwt}` } })
        .then(response => {
            // JSON responses are automatically parsed.
            //console.log(response.data)
            var todos = response.data.users
    ...
    created() {
        ...
        axios.get(`${process.env.VUE_APP_BACKEND}/users/users?nome=${this.$store.state.user._id}&type=Student`, { headers: { Authorization: `Bearer: ${this.$store.state.jwt}` } })
        .then(response => {
            // JSON responses are automatically parsed.
            //console.log(response.data)
            var todos = response.data.users
