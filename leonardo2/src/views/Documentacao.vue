...
export default {
    ...
    methods: {
      atualizarInfo: function(){
        this.dialog=false
        axios.get(`https://leonardo2.di.uminho.pt/api/documentacao/docs?nome=${this.$store.state.user._id}`, { headers: { Authorization: `Bearer: ${this.$store.state.jwt}` } })
          .then(response => {
      ...
      verObjectItem(item){
        ...
        // if (value == 'curriculo'){
          axios.get(`https://leonardo2.di.uminho.pt/api/documentacao/ficheiro/${this.docs[index]._id}?seed=${Date.now()}`, {
      ...
      deleteItem (item) {
        ...
        axios.get(`https://leonardo2.di.uminho.pt/api/documentacao/apagar/` + this.docs[index]._id + `?nome=${this.$store.state.user._id}`,{ headers: { Authorization: `Bearer: ${this.$store.state.jwt}` } })
        .then(response => {
            // JSON responses are automatically parsed.
    ...
    created() {
        ...
        axios.get(`https://leonardo2.di.uminho.pt/api/documentacao/docs?nome=admin`, { headers: { Authorization: `Bearer: ${this.$store.state.jwt}` } })
        .then(response => {
            // JSON responses are automatically parsed.
            this.docs = response.data.docs
