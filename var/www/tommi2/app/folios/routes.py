...
@blueprint.route('/folios')
@login_required
def route_template_folios():
    folios = mongo.db.folios.find()
    nome = request.args.get('nome')
    return render_template('folios.html',folios=folios,nome=nome)

@blueprint.route('/folios/intrepretativa')
@login_required
...
@blueprint.route('/ver/<folio>/foto')
@login_required
def route_template_ver_foto(folio):
    nome = request.args.get('nome')
    existe = mongo.db.folios.find_one({"_id":folio})
    fotos = listdir(join(dirname(realpath(__file__)), 'static/pics/'))
    if folio in fotos:
        return render_template('viewFolioFoto.html',folio=existe,foto=1,nome=nome)
    else:
        return render_template('viewFolioFoto.html',folio=existe,nome=nome)
...
def route_template_index():
    ...
    return render_template('indices.html',indexs=index,nome=nome)

@blueprint.route('/index/ver/<indice>')
@login_required
...
def route_template_tags():
    tags = mongo.db.tags.find()
    nome = request.args.get('nome')
    return render_template('tags.html',tags=tags,nome=nome)

@blueprint.route('/tags/ver/<tag>')
@login_required
